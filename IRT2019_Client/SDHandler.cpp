...
void SDHandler::write(char filename[], int rep, float battV, tmElements_t & tm, float dataIRT[3][2]) {
    File file = SD.open(filename, FILE_WRITE);
    if (file && SD.exists("/TEST")) {
...
// write str to Flash
/*void SDHandler::preserve(int rep, float battV, tmElements_t & tm, float dataIRT[3][2]) {
    // put date into block
    Block block;
    block.rep = rep;
    block.battV = battV;
...
void SDHandler::checkHeader(char filename[]) {
    ...
    if (first == -1) {
        ...
        myfile.println(F("Rep,Battery Voltage(V),Timestamp,Ground Object T (deg C),Ground Ambient T (deg C),Leaf T (deg C),Leaf Ambient T (deg C),Canopy T (deg C),Ambient Air (deg C)"));
        myfile.close();
    }
